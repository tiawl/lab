...
container_create () {
  ...
  img="${project[image]}${1}"
  json="{\"Hostname\":\"${1}\",\"Image\":\"${img}${sep[tag]}$(image tag list "${img}")\"}"
  ...
  curl --silent --show-error --request "${method}" --unix-socket "${path[socket]}" --header 'Content-Type: application/json' --data "${json}" --write-out "%{stderr}$(printf "%$(( ${#REPLY[req_id]} + 1 ))s")> HTTP %{response_code}\n" "${endpoint}" \
    | jq '.'
}
